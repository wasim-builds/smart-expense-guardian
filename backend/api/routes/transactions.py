...
def add_transaction(tx: schemas.TransactionCreate, db: Session = Depends(get_db)):
    ...
    return transaction_repo.create_transaction(db, tx, category, is_anomaly, anomaly_score)
...
def read_transactions(
    ...
    txs = transaction_repo.get_transactions(db, account_name=account_name, search=search, category=category)
    return txs[skip : skip + limit]

@router.post("/upload")
async def upload_transactions(file: UploadFile = File(...), account_name: str = Form("Main Account")):
    ...
    task = process_csv_batch.delay(content_str, account_name)
...
@router.get("/export")
def export_transactions(account_name: Optional[str] = Query(None, description="Filter by account"), db: Session = Depends(get_db)):
